...
function Card({ data, formattedDate, active }) {
  console.log(typeof data);
  // console.log(data?.name);
  // const cardClassName = active ? "card-dark" : "card-light";
  return (
    <>
      <CardStyle active={active}>
        ...
        <Detailsbox>
            ...
            <UserDetails>
              <UserNameBox>
                <UserName active={active}>
                  {data == false ? "The Octocat" : data?.name}
                </UserName>
                <Company>@{data == false ? "octocat" : data?.login}</Company>
              </UserNameBox>
              <JoinDate active={active}>
                Joined {data == false ? "25 Jan 2011" : formattedDate}
              </JoinDate>
            </UserDetails>
          </UserBio>
          ...
          <CommunityBox active={active}>
            <RepoBox>
              <Repos active={active}>Repos</Repos>
              <ReposNumber active={active}>
                {data == false ? "8" : data?.public_repos}
              </ReposNumber>
            </RepoBox>
            <FollowersBox>
              <Followers active={active}>Followers</Followers>
              <FollowersNumber active={active}>
                {data == false ? "3938" : data?.followers}
              </FollowersNumber>
            </FollowersBox>
            <FollowingBox>
              <Following active={active}>Following</Following>
              <FollowingNumber active={active}>
                {data == false ? "9" : data?.following}
              </FollowingNumber>
            </FollowingBox>
          </CommunityBox>
          <ContactDetails>
                ...
                <path
                  ...
                  fill={
                    active && data?.location
                      ? "#4b6a9b"
                      : active && !data?.location
                      ? "rgba(75, 106, 155, 0.49)"
                      : !active && data?.location
                      ? "#FFFFFF"
                      : !active && !data?.location
                      ? "rgba(255, 255, 255, 0.496)"
                      : null
                  }
                />
              </svg>
              <Location active={active} data={data}>
                {data == false
                  ? "San Francisco"
                  : data?.location
                  ? data.location
            ...
            <BlogBox>
              ...
              <Blog
                active={active}
                data={data}
                href={data == false ? "https://github.blog" : data?.blog}
              >
                {data == false
                  ? "https://github.blog"
                  : data?.blog
            ...
            <TwitterBox>
              ...
              <Twitter active={active} data={data}>
